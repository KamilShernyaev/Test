...
namespace SG
{
    public class AttackState : State
    {
        ...
        public override State Tick(EnemyManager enemyManager, EnemyStats enemyStats, EnemyAnimatorManager enemyAnimatorManager)
        {
            ...
            {
                if(enemyManager.distanceFromTarget < currentAttack.mininumDistanceNeededToAttack)
                {
                    return this;
                }
         ...
         private void GetNewAttack(EnemyManager enemyManager)
        {
            ...
            for (int i = 0; i < enemyAttacks.Length; i++)
            {
                ...
                if (enemyManager.distanceFromTarget <= enemyAttackAction.maximumDistanceNeededToAttack && enemyManager.distanceFromTarget >= enemyAttackAction.mininumDistanceNeededToAttack)
                {
            ...
            for (int i = 0; i < enemyAttacks.Length; i++)
            {
                ...
                && enemyManager.distanceFromTarget >= enemyAttackAction.mininumDistanceNeededToAttack)
                {
                    if (viewableAngle <= enemyAttackAction.maximumAttackAngle 
                    && viewableAngle >= enemyAttackAction.minimumAttackAngle)
                    {
